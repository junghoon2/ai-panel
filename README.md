...
- **파일 쓰기(선택)** — `/write` 로 claude 에 편집 권한을 부여하면 답변에 그치지 않고 파일을 직접 수정 (기본은 읽기 전용)
...
| `/write` | claude 에 **파일 쓰기 권한** 부여 — 파일 생성·편집 자동 승인 (헤더에 빨간 `[claude 쓰기]` 표시). 다음 질문부터 적용 |
| `/readonly` | claude 쓰기 권한 해제 — 기본 **읽기 전용**으로 복귀 |
...
- 기본은 **읽기 전용**입니다. 헤드리스 모드에는 권한 승인 다이얼로그를 띄울 곳이 없어 파일 쓰기가 거부됩니다. `/write` 를 켜면 claude 워커가 `--permission-mode acceptEdits` 로 재기동되어(세션은 `--resume` 으로 유지) 파일 생성·편집을 자동 승인합니다. Bash·삭제 등 더 위험한 작업은 여전히 막힙니다. `/all` 상태에서는 도구 간 파일 경합을 피하려 `/claude` 전용 모드 사용을 권장합니다
