...
- **응답 스크롤** — 답변이 길어 잘리면 `Shift+↑↓` 로 현재 턴 패널을 거슬러 확인 (세 패널 동시)
- **대화 히스토리** — 지나간 턴은 터미널 스크롤백에 남아 위로 스크롤하면 확인 가능
- **파일 쓰기(선택)** — `/claude 쓰기 권한 추가해줘` 로 claude(또는 `/codex`)에 편집 권한을 부여하면 답변에 그치지 않고 파일을 직접 수정 (기본은 읽기 전용)
...
| `/claude 쓰기 권한 추가해줘` | claude 에 **파일 쓰기 권한** 부여 — 파일 생성·편집 자동 승인 (헤더에 빨간 `[claude 쓰기]` 표시). 다음 질문부터 적용. `/codex` 도 동일 (gemini 제외) |
| `/claude 쓰기 권한 해제` | claude 쓰기 권한 해제 — 기본 **읽기 전용**으로 복귀. `/codex` 동일 |
| `/review <리뷰어> <대상>` | 리뷰어가 대상 도구의 답변을 리뷰 (예: `/review claude gemini`) |
| `/review all` | 각 도구가 나머지 도구들의 답변을 교차 리뷰 (3패널 동시) |
| `/exit` (별칭: `/quit`, `/q`) | 종료 |
| `Shift+↑` `Shift+↓` | **현재 턴 패널 스크롤** — 답변이 길어 잘렸을 때 위/아래로 거슬러 확인 (세 패널 동시, 가려진 줄은 헤더에 `▲/▼` 표시) |
| `ESC` | 스크롤 중이면 **최신으로 복귀** → 아니면 **응답 중단**(진행 중인 턴 취소, 세션 유지) → 입력 비우기 |
...
- **커서 편집**: ←→ 로 커서 이동 후 중간 수정, 여러 줄에서는 ↑↓ 로 줄 간 이동
- **응답 스크롤**: `Shift+↑↓` 로 현재 턴의 세 패널을 함께 위/아래로 거슬러 봅니다. 가려진 줄 수는 헤더에 `▲/▼` 로 표시되고, `Esc` 또는 새 질문 시 최신으로 복귀합니다 (터미널이 Shift+화살표를 표준 시퀀스로 보낼 때 동작)
- 답변이 진행 중에도 입력·편집은 가능합니다. Enter 로 보내면 전송 대기 큐에 쌓였다가 현재 턴이 끝나면 자동 전송됩니다
...
- 기본은 **읽기 전용**입니다. 헤드리스 모드에는 권한 승인 다이얼로그를 띄울 곳이 없어 파일 쓰기가 거부됩니다. `/claude 쓰기 권한 추가해줘` 로 켜면 claude 워커가 `--permission-mode acceptEdits` 로 재기동되어(세션은 `--resume` 으로 유지) 파일 생성·편집을 자동 승인합니다. `/codex` 도 동일하게 `--sandbox workspace-write` 로 워크스페이스 쓰기를 허용합니다(gemini 는 미지원). Bash·삭제 등 더 위험한 작업은 여전히 막힙니다. `/all` 상태에서는 도구 간 파일 경합을 피하려 전용 모드 사용을 권장합니다
...
| 답변이 잘려 보임 | 패널은 최신 내용 우선(tail) 표시. **현재 턴**은 `Shift+↑↓` 로 거슬러 보고, 지나간 턴은 스크롤백(위로 스크롤)에서 확인 |
| `Shift+↑↓` 스크롤이 안 됨 | 터미널이 Shift+화살표를 가로채는 경우. 대부분(iTerm2 등)은 동작하나, 안 되면 터미널 설정에서 Shift+화살표 매핑 해제 필요 |
